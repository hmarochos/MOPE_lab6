...
def fisher_value(f3, f4, significance):
...
def find_average(lst, orientation):
    """Пошук середнього значення по колонках або по рядках"""
    average = []
...
def fisher_test():
    ...
    F_theoretical = fisher_value(f3, f4, q)
...
for i in range(0, 100):
    ...
    while not adequacy:
        ...
        k = [my, find_known(1), find_known(2), find_known(3), find_known(4), find_known(5), find_known(6),
                 find_known(7),
        ...
        while not homogeneity:
            print(" " * 49 + "Матриця планування експеременту" + " " * 49 + " " + "\n")
            print("      x1       x2        x3      x1x2      x1x3      x2x3     x1x2x3      x1x1     x2x2       x3x3                Yi")
            for row in range(N):
                print(end=' ')
                for column in range(len(matrix[0])):
                    print("{:9.2f}".format(matrix[row][column]), end=' ')
        ...
        for i in range(N):
            print("y{} = {:.3f} ≈ {:.3f}".format((i + 1), check_result(student_lst, i), average_y[i]))
        ...
        print("Час роботи програми {:.5f} секунд".format(cycleTime))

middleTime = cycleTime / 100
print("Середній час виконання циклу - {:.5f} секунди".format(middleTime))
